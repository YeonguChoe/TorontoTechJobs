...
export default function Post() {
    ...
    useEffect(() => {
        axios
            .get(`${process.env.SERVER_ADDRESS}/jobs/${postID}`)
            .then((res) => {
                setPostDetail(res.data);
                // After setting postDetail, make the second axios.get request
                return axios.get(
                    `${process.env.SERVER_ADDRESS}/companies/filter-by-company-name?company_name=${res.data.company_name}`
                );
            })
            .then((res) => {
    ...
    function handleDelete() {
        ...
        axios
            .delete(`${process.env.SERVER_ADDRESS}/jobs/${postID}`, configuration)
            .then((res) => {
                    ...
                    <div className="col-3">
                        <div className="card">
                            <div className="card-body">
                                ...
                                <p>
                                    Apply at:{" "}
                                    <a href={`mailto:${companyDetail.email}`}>
                                        {" "}
                                        {companyDetail.email}{" "}
                                    </a>
                                </p>
